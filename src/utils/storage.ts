...
export function getStoredRestaurantLogo(): string | null {
  try {
    return localStorage.getItem(STORAGE_KEYS.RESTAURANT_LOGO) || null;
  } catch (err) {
    console.error('Error reading stored logo', err);
    return null;
  }
}

export function setStoredRestaurantLogo(logoUrl: string | null): void {
  try {
    if (logoUrl && logoUrl.trim()) {
      localStorage.setItem(STORAGE_KEYS.RESTAURANT_LOGO, logoUrl.trim());
    } else {
      localStorage.removeItem(STORAGE_KEYS.RESTAURANT_LOGO);
    }
  } catch (err) {
    console.error('Error saving restaurant logo', err);
  }
}

export function getStoredHeroBg(): string | null {
  try {
    return localStorage.getItem(STORAGE_KEYS.HERO_BG) || null;
  } catch (err) {
    console.error('Error reading stored hero bg', err);
    return null;
  }
}

export function setStoredHeroBg(bgUrl: string | null): void {
  try {
    if (bgUrl && bgUrl.trim()) {
      localStorage.setItem(STORAGE_KEYS.HERO_BG, bgUrl.trim());
    } else {
      localStorage.removeItem(STORAGE_KEYS.HERO_BG);
    }
  } catch (err) {
    console.error('Error saving hero bg', err);
  }
}
